...
LL-REACH VARIANT: adds a DENSE gripper->cube reach term to the LOW-LEVEL reward ONLY
(r_int): reward += reach_w*(dist_t - dist_{t+1}) - reach_pot_w*dist_{t+1}, dist read directly
from the obs (box_pos-gripper_pos vector at [46:49]). It does NOT touch the HL
value/hindsight or the JEPA loss, so all 4 faithfulness properties and the HL
objective are unchanged. Tests whether a learnable LL reach primitive unblocks the
H-JEPA hierarchy (reached>0 -> success>0?). reach_w=0 reproduces the 0.0 baseline.

RAW-GEO VARIANT (Bet-2 FINAL test): combines BOTH wall fixes.
  (a) ACTION:  OSC/Cartesian LL action (from osc_env) — reach is FEASIBLE.
  (b) PERCEPTION: the LL actor/critic are additionally fed a RAW GEOMETRY vector
      taken DIRECTLY from the obs (NOT routed through the JEPA encoder), so the LL
      can reach RELIABLY (the osc-only LL acted on JEPA latents that don't preserve
      fine ee-cube geometry, so reach was feasible-but-unreliable ~0.03).
  raw_geo(o) = concat([ obs[46:49]=box_pos-gripper_pos (REACH vec),
                        obs[49:52]=target_pos-box_pos (PLACE vec),
                        obs[31:34]=gripper_pos (absolute ee pos) ])  -> 9-dim.
  The LL input becomes (z, g, raw_geo) instead of (z, g). The HL is UNCHANGED: it
  still emits a LATENT subgoal g from the JEPA latent z (latent subgoals preserved).
  The JEPA encoder/predictor/loss, EMA target, VICReg, 2-level k-jump, non-generative
  / no-decoder HL — ALL untouched. Only the LL's *input* gains raw geometry.
  Tests the FINAL Bet-2 question: with reach made both FEASIBLE (OSC) and RELIABLE
  (raw-geo input), does the learned latent H-JEPA hierarchy cross competence?
"""
...
sys.path.insert(0, str(HELIOS / "hl_pickcube"))
from mujoco_playground import registry, wrapper  # noqa: E402
import osc_env as OSC  # noqa: E402  (Cartesian/OSC ee-action wrapper)
...
def main():
    ...
    # LL-ONLY DENSE REACH SHAPING (the mechanism under test). Negative gripper->cube
    # distance + its per-step reduction, added ONLY to the LL reward r_int. NOT fed to
    # the HL value/hindsight and NOT to the JEPA loss. obs[reach_idx0:reach_idx0+3] is
    # (box_pos - gripper_pos) for PandaPickCube (verified); reach dist = its L2 norm.
    ap.add_argument("--reach_w", type=float, default=30.0, help="LL-only reach REDUCTION weight (per-step dist drop). Raised 1->30 so the ~0.04m/step reach signal is not swamped by the O(1)-normalized subgoal term.")
    ap.add_argument("--reach_pot_w", type=float, default=2.0, help="LL-only reach POTENTIAL weight (-dist, dense pull toward contact)")
    ap.add_argument("--reach_idx0", type=int, default=46, help="obs start index of (box_pos-gripper_pos) 3-vec")
    # RAW-GEOMETRY LL INPUT (the perception fix under test). Feed these obs slices
    # DIRECTLY into the LL actor/critic input (concatenated to (z,g)) — NOT through
    # the JEPA encoder. obs[46:49]=box-grip (reach), obs[49:52]=target-box (place),
    # obs[31:34]=gripper_pos. Verified layout: nq=16,nv=15 so grip_pos@31, reach@46.
    ap.add_argument("--rawgeo_slices", default="46:52,31:34",
                    help="comma-separated obs slices fed raw to the LL (a:b). "
                         "Default: reach+place (46:52) and gripper_pos (31:34) -> 9 dims.")
    ...
    # ── Env (OSC: LL acts in Cartesian ee-displacement space via DLS IK)
    env = OSC.make_osc_env(config_overrides={"impl": "jax"})
    ...
    # ── RAW-GEO LL input: parse obs slices, build a jit-able raw_geo(o) selector.
    # raw_geo is fed DIRECTLY to the LL actor/critic (NOT through the JEPA encoder).
    _rg_bounds = []
    for s in args.rawgeo_slices.split(","):
        a_, b_ = s.split(":")
        _rg_bounds.append((int(a_), int(b_)))
    RG_IDX = np.concatenate([np.arange(a_, b_) for (a_, b_) in _rg_bounds]).astype(np.int32)
    RG_IDX_J = jnp.asarray(RG_IDX)
    RG_DIM = int(RG_IDX.shape[0])

    def raw_geo(o):
        """Select the raw-geometry obs dims for the LL input (NOT encoded)."""
        return jnp.take(o, RG_IDX_J, axis=-1)

    LL_IN = 2 * LD + RG_DIM   # LL actor/critic input width = (z | g | raw_geo)
    print(f"[hjepa] env={args.env} obs={obs_dim} act={act_dim} N={N} latent={LD} k={K} "
          f"rawgeo_dims={RG_DIM} idx={RG_IDX.tolist()} ll_in={LL_IN}", flush=True)
    ...
    hl = HLPolicy(LD, V=V)
    ll_pi = Pi(act_dim)                     # LL actor: input (z|g|raw_geo)
    ll_q = QEnsemble()                      # LL critic: input (z|g|raw_geo), a
    ...
    p_pi = ll_pi.init(ks[3], jnp.zeros((1, LL_IN)))
    p_q = ll_q.init(ks[4], jnp.zeros((1, LL_IN)), jnp.zeros((1, act_dim)))
    ...
    @jax.jit
    def ll_act(p, z, g, rg, key, noise):
        zg = jnp.concatenate([z, g, rg], -1)        # raw geometry concatenated raw
    ...
    @jax.jit
    def ll_act_det(p, z, g, rg):
        mean, _ = ll_pi.apply(p["pi"], jnp.concatenate([z, g, rg], -1))
    ...
    def ll_loss(params, target, o, a, r_int, o2, g, done):
        ...
        RAW-GEO: the LL input also includes raw_geo(o) taken DIRECTLY from the obs
        (not through the encoder), so the LL can reach reliably."""
        ...
        rg = raw_geo(o)                                          # raw geometry (not encoded)
        rg2 = raw_geo(o2)
        zg = jnp.concatenate([z, g, rg], -1)
        zg2 = jnp.concatenate([z2, g, rg2], -1)
        # target Q
    ...
    def sample_batch():
        ...
        # ── LL-ONLY DENSE REACH SHAPING (mechanism under test) ──────────────────
        # gripper->cube distance read directly from the obs (box_pos-gripper_pos at
        # [reach_idx0:reach_idx0+3]); reward = reduction (dist_t - dist_{t+1}) plus a
        # small negative-distance potential. Added ONLY to r_int (the LL SAC reward).
        # The HL hindsight uses env_ret (below, untouched) and the JEPA loss uses no
        # reward at all, so all 4 faithfulness properties + the HL objective stand.
        i0 = args.reach_idx0
        rd1 = np.linalg.norm(np.asarray(o)[:, i0:i0 + 3], axis=-1)
        rd2 = np.linalg.norm(np.asarray(o2)[:, i0:i0 + 3], axis=-1)
        # reach reduction (dist_t - dist_{t+1}) is ~+/-0.04m/step; the subgoal term
        # above is normalized to O(1)/step, so reach_w (~30) is needed for the reach
        # signal to drive the LL. reach_pot_w*(-dist) is a dense pull toward contact.
        r_int = r_int + args.reach_w * (rd1 - rd2) - args.reach_pot_w * rd2
        # HL hindsight
    ...
    def _eval_rollout(params, est0, T):
        ...
        def body(carry, _t):
            ...
            a = ll_act_det(params, z, g, raw_geo(o))   # raw geometry from current obs
            est = _step(est, a)
    ...
    while env_steps < args.total_steps:
        ...
        if env_steps < args.warmup:
            ...
            a = ll_act(params, z, cur_g, raw_geo(jnp.asarray(st.obs)), ak, noise)
        a_np = np.asarray(a)
        nst = _step(st, a)
    ...
    if metrics_log:
        ...
        summary = {
            "run": "hjepa_osc_rawgeo", "env": args.env, "seed": args.seed,
            "rawgeo_dims": RG_DIM, "rawgeo_idx": RG_IDX.tolist(),
