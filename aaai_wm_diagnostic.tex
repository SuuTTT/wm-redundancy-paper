...
For Dreamer we report the last-30-episode median return at $\sim$1.1M
environment steps; for TD-MPC2 the certified JAX reimplementation matches the
official release (cheetah $\sim$919, hopper $\sim$581 for the reference
implementation vs.\ 855/455 for ours; reimplementation certified). The strip
ablation removes only the forward-dynamics learning signal.
...
negative.} We include it because it clarifies the contribution: this paper
offers a \emph{diagnostic}, not a repaired algorithm. Knowing \emph{whether}
a WM will help is separable from---and, on the evidence here, easier than---
\emph{fixing} a WM that does not.
