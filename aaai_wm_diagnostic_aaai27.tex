...
\title{When Does a Learned World Model Help?\\
...
return. Across nine DeepMind Control tasks and two independent WM families
(Dreamer, TD-MPC2) the world model's value ranges from \emph{essential}
(stripping it collapses return to near zero on ball-in-cup,
...
\item A \textbf{cross-model, cross-task} result over nine DMControl tasks:
...
\midrule
\textsc{ball-in-cup catch} & 972.0 & \textbf{0.0}  & \textbf{collapse} & essential$^\dagger$ \\
\textsc{pendulum-swingup}  & 803.0 & 242 (bimodal)  & $-70\%$ & ess.$^\dagger$/bimodal \\
...
\textsc{quadruped-walk}    & 958.1 & 963.7         & $+1\%$            & redundant \\
\bottomrule
\end{tabular}
\caption{\textbf{Nine-task WM-dependence gradient} (Dreamer, last-30-episode
...
removed; negative means the stripped agent is \emph{better}. Acrobot, cheetah,
walker are $n{=}3$; reacher-hard and pendulum $n{=}2$--$3$; the rest single-seed.
$^\dagger$ marks the two \emph{exploration-influenced} tasks (ball-in-cup,
pendulum): their collapse is a data-collection failure, not value-insufficiency
(Sec.~\ref{sec:cross}, boundary), so they are excluded from the value-limited
correlation. Pendulum is \emph{bimodal} across seeds (strip $0/727/0$)---it
either fails to discover the swing-up or matches vanilla---hence the $-70\%$
mean is not a graded deficit. The essential end is sparse-reward / long-horizon
...
not a property of the architecture but of the \emph{task}: it spans near-total
collapse (ball-in-cup and reacher-hard score near zero when stripped) to
strip-wins (three tasks improve), with a smooth middle. Colour encodes regime;
pendulum's bar is its bimodal $n{=}3$ mean (see Table~\ref{tab:cross}).}
\label{fig:gradient}
\end{figure}
...
($26,56,100$); on cup-catch and reacher-hard the stripped agent scores near
zero for the entire run. Pendulum is the same pattern taken to the extreme---
bimodal across seeds (strip $0/727/0$)---which is why we treat it, with
cup-catch, as exploration-influenced rather than value-limited (below).
...
value-limited tasks with both measurements, the VBN value-recovery at a narrow
bottleneck ($D{=}16$, a scalar read of compressibility) against WM-dependence.
The relationship is strongly monotone and negative (Spearman
$\rho{=}{-}0.90$, Pearson $r{=}{-}0.79$, $n{=}5$): tasks whose value is already
...
\paragraph{Where the diagnostic stops: an honest boundary.}
One task breaks the correlation, and it is informative. \textsc{ball-in-cup}'s
value is \emph{maximally} compressible in TD-MPC2---a $D{=}16$ bottleneck
recovers $\sim$100\% of its unbottlenecked return ($975$ vs.\ a $974$
ceiling)---yet stripping the WM \emph{collapses} it in Dreamer (return $0$).
By value-compressibility it should be WM-redundant; by ablation it is
WM-essential. Including it drops the rank correlation to $\rho{=}{-}0.21$
($n{=}7$), and we report both numbers rather than dropping the point. We read
ball-in-cup as the diagnostic's \emph{boundary}: VBN measures whether the
\emph{value function} needs the full latent, so it predicts
\emph{value-limited} WM-dependence. Ball-in-cup's Dreamer collapse is instead
\emph{exploration-limited}---without imagined rollouts the agent never
discovers the catch in the first place, a data-collection failure that a
value-sufficiency probe is not designed to see. The diagnostic is thus scoped
to tasks where the WM's role is representational rather than exploratory; the
$\rho{=}{-}0.90$ core holds cleanly within that scope.
\textsc{Pendulum-swingup} is a second, milder case of the same effect: its
stripped ablation is \emph{bimodal} across seeds (strip returns $0/727/0$ at
$n{=}3$ versus vanilla $\sim$$800$)---the agent either never discovers the
swing-up or matches vanilla with no representational deficit---so its
WM-dependence is exploration-influenced rather than value-limited, and we
exclude it from the value-limited core as well. (It is less extreme than
ball-in-cup: SAC reaches $575$ on pendulum vs.\ $\sim$$980$ on ball-in-cup,
so pendulum is only partly model-free-solvable.)

\paragraph{An independent cross-check from a third method.}
The exploration-limited boundary is corroborated by data we did not generate:
the official TD-MPC2 release \cite{hansen2024tdmpc2} also reports SAC, a
model-free off-policy learner with \emph{no} world model at all. On the tasks
we label exploration-limited, SAC nonetheless solves them---the
TD-MPC2$-$SAC final-return gap is only $+5$ on ball-in-cup ($984$ vs.\ $979$)
and $+38$ on reacher-hard ($982$ vs.\ $944$). Their Dreamer collapse therefore
cannot be a value-representation deficit; it is an exploration/reconstruction
failure specific to Dreamer's generative objective---precisely the boundary a
value probe cannot see. The genuinely value- and planning-hard tasks instead
show large gaps (acrobot $+591$, hopper-hop $+332$). One caution, which opens a
separate question we leave to future work: on hopper-hop the gap is $+332$ even
though TD-MPC2's self-predictive (consistency) loss is \emph{removable} there
and plan-time lookahead adds nothing---so what beats SAC on hopper-hop is
TD-MPC2's value pathway, not its world model. Here this third signal serves
only to confirm that our exploration-limited tasks are model-free--solvable,
hence not value-limited.
...
world-model dependence (Dreamer strip ablation) across five value-limited tasks, cross-model
($\rho{=}{-}0.90$). Compressible value $\Rightarrow$ redundant world model. The
open point is \textsc{ball-in-cup}, the one cross-model outlier: compressible
in TD-MPC2 yet Dreamer-essential because its failure there is exploratory, not
value-limited (see text). It is shown, not hidden; the fit is on the core five
value-limited tasks (pendulum-swingup is likewise excluded as exploration-influenced).}
...
\paragraph{Why a value-sufficiency probe has a boundary.}
The ball-in-cup exception (Sec.~\ref{sec:cross}) is not noise but a consequence
of \emph{what} VBN measures, and it is worth making precise. Decompose the
benefit of a learned WM into two channels: a \emph{representational} channel,
in which the WM shapes the latent so the value and policy heads can express and
propagate long-horizon credit; and an \emph{exploratory} channel, in which
imagined rollouts steer data collection toward states the policy would
otherwise never visit. Writing the achieved return as
$J \approx J_{\text{rep}}(\phi) + J_{\text{expl}}(\mathcal{D})$---a term set by
the representation on a fixed dataset and a term set by the induced state
occupancy $\mathcal{D}$---VBN instruments only $J_{\text{rep}}$: it freezes
$\phi$ and $\mathcal{D}$ and asks whether the value head needs the full latent.
On tasks whose difficulty is representational (\textsc{acrobot},
\textsc{pendulum}, \textsc{reacher}) the two channels move together and VBN
predicts WM-dependence tightly. \textsc{Ball-in-cup} is the corner case where
the value is trivially compressible \emph{once the goal region is reached}
(so $J_{\text{rep}}$ is bottleneck-insensitive, giving $\sim$100\% recovery)
yet the goal is almost never reached without imagined rollouts (so
$J_{\text{expl}}$ dominates the ablation). A value-sufficiency probe is silent
on $J_{\text{expl}}$ by construction; a complete a-priori criterion needs a
second, exploration-side statistic---a hitting-time or reward-sparsity measure
of the induced occupancy---and the reward-propagation horizon above is a
natural candidate because it couples to both. We state this as the precise
scope of the diagnostic, not a defect: VBN answers ``does the value function
need the world model to \emph{represent} return?'', which is exactly the
question a practitioner deciding whether to pay for one wants answered on the
large class of representation-limited tasks.
