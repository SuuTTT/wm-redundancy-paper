...
correlates with world-model dependence at Spearman $\rho{=}{-}0.94$
($n{=}6$), \emph{cross-model}. We also report a clean \emph{negative}:
...
predicted by the VBN fingerprint at Spearman $\rho{=}{-}0.94$
...
ablation and \emph{predicts} it. Fig.~\ref{fig:correlation} plots, for the six
...
$\rho{=}{-}0.94$, Pearson $r{=}{-}0.87$, $n{=}6$): tasks whose value is already
...
$\rho{=}{-}0.94$ core holds cleanly within that scope.
...
world-model dependence (Dreamer strip ablation) across six tasks, cross-model
($\rho{=}{-}0.94$). Compressible value $\Rightarrow$ redundant world model. The
...
value-limited (see text). It is shown, not hidden; the fit is on the core six.}
